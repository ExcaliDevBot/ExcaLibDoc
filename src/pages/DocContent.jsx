        ...
        <section key={index} className="mb-12">
          ...
          {section.content.map((item, itemIndex) => {
            switch (item.type) {
              ...
              case 'callout':
                return (
                  <CalloutBox key={itemIndex} type={item.calloutType} title={item.title}>
                    {item.text}
                  </CalloutBox>
                );
              case 'list':
                return item.ordered ? (
                  <ol key={itemIndex} className="list-decimal pl-6 mb-6 text-slate-700 dark:text-slate-300 space-y-2">
                    {item.items.map((listItem, listItemIndex) => (
                      <li key={listItemIndex}>{listItem}</li>
                    ))}
                  </ol>
                ) : (
                  <ul key={itemIndex} className="list-disc pl-6 mb-6 text-slate-700 dark:text-slate-300 space-y-2">
                    {item.items.map((listItem, listItemIndex) => (
                      <li key={listItemIndex}>{listItem}</li>
                    ))}
                  </ul>
